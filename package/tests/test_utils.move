...
public fun default_game(ctx: &mut TxContext): (Game, House, HouseAdminCap, ParameterStore, GameStatistics) {
    let coin_flip_cap = get_admin_cap_for_testing(ctx);
    let mut registry = registry_for_testing(ctx);
    let (game, param_store, stats) = game::admin_create(&coin_flip_cap, &mut registry, 0, 10_000_000, 2_000, 20_000, ctx);
